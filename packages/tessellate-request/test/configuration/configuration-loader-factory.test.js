...
describe('configuration loader factory', () => {
  it('throws an error when AUTH_CONFIGURATION is missing', () => {
    expect(() => configurationLoaderFactory.create('file:///config.json')).toThrowError(
      'Tried to create configuration loader, but AUTH_CONFIGURATION is missing'
    );
  });
  ...
  it('creates file configuration loader', () => {
    ...
    expect(configurationLoader).toBeInstanceOf(FileConfigurationLoader);
  });

  it('throws an error when url type is not supported', () => {
    nconf.set('AUTH_CONFIGURATION', 'protocol://config.json');
    expect(() => configurationLoaderFactory.create('protocol://config.json')).toThrowError(
      'Unable to load auth configuration from protocol://config.json'
    );
  });
});
